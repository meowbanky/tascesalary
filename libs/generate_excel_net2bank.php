...
if($bank != -1){
    ...
    // Set header row
    $headers = ['S/N', 'NAME', 'AMOUNT', 'PAYMENT DATE','BENEFICIARY CODE','ACCOUNT','BANK CODE','DEBIT ACCOUNT','BANK','STATUS'];
    ...
    foreach($GrossPays as $GrossPay)
        {
            ...
            $sheet->setCellValue('H' . $row, '1229191715');
            $sheet->setCellValue('I' . $row, $GrossPay['bankname']);
            $sheet->setCellValue('I' . $row, $GrossPay['STATUSCD']);
