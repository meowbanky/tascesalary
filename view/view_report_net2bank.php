...
<div class="bg-white p-6 rounded-lg shadow-md">
    <div class="container mx-auto py-8">
        <div class="flex justify-between">
            <h1 class="text-2xl font-bold mb-6">Net to Bank</h1>
            <div>
            ...
            <div class="bg-white shadow rounded-lg p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <h2 class="text-lg font-semibold">Net to Bank Report</h2>
                    </div>
                    <div class="flex items-center space-x-4">
                ...
                <div class="mb-4">
                    ...
                    <select id="bank" name="bank" class="w-full mt-1 border border-gray-300 rounded-md p-2">
                        <option value="">Select Bank</option>
                        <option value="-1">Select All Banks</option>
                        <?php
                        foreach ($banks as $bank) {
                            echo "<option value='" . $bank['BANK_ID'] . "'>" . $bank['BNAME'] . '</option>';
                        }
                        ?>
                    </select>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#submit').click(function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: 'libs/get_report_net2bank.php',
                data: {
        ...
        $('#download-excel-button').click(function() {
            ...
            var bank = $('#bank').val();
            var email = $('#email').val();
            window.location.href = 'libs/generate_excel_net2bank.php?payperiod=' + period + '&bank=' + bank + (email ? '&email=' + encodeURIComponent(email) : '');
        });
        ...
        $('#export-pdf-button').click(function() {
            ...
            var bank = $('#bank').val();
            var email = $('#email').val();
            window.location.href = 'libs/generate_pdf_net2bank.php?payperiod=' + period + '&bank=' + bank + (email ? '&email=' + encodeURIComponent(email) : '');
        });
    });
</script>
